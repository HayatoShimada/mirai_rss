...
## 主な機能
- **RSS & HTML自動解析**: RSSフィードからの高速な取得はもちろん、RSSがないサイトのHTMLページもGeminiモデル（Google Search Grounding）を用いて自律的に内容を読み取ります。
- **高精度な要約・フィルタリング**: 1日分のニュースから重複を排除し、「ニュース」「補助金・法律」「イベント」のカテゴリ付きで、最大5件の重要な情報を要約して配信します。
